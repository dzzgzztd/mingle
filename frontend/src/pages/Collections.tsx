...
export default function Collections() {
    ...
    const load = async () => {
        ...
        try {
            ...
            setError(e?.response?.data?.error || "Не удалось загрузить подборки");
            setItems([]);
        } finally {
            setLoading(false);
        }
    ...
    const add = async () => {
        ...
        try {
            ...
            setError(e?.response?.data?.error || "Не удалось создать подборку");
        } finally {
            setCreating(false);
        }
    };
    ...
    const loadRecs = async (id: number) => {
        ...
        try {
            ...
            setRecsById((prev) => ({
                ...prev,
                [id]: res.data.recommendations || [],
            }));
        } catch (e: any) {
            setError(e?.response?.data?.error || "Не удалось получить рекомендации");
        }
    };
    ...
    return (
        <div>
            ...
            {error && <div className="small">ошибка: {error}</div>}
            ...
            {!loading && empty && <div className="small">пока нет подборок</div>}
        </div>
    );
}